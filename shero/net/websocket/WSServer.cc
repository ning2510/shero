...
void WSServer::HttpHandShake(const TcpConnectionPtr &conn, Buffer *buf) {
    TcpConnection *it = conn.get();
    http::HttpRequest::ptr req = http::HttpServer::HttpParserRequest(buf);
    if(!req) {
        LOG_ERROR << "[WSServer] invalid http request";
        return ;
    }
    ...
    http::HttpResponse::ptr res(
        new http::HttpResponse(req->getVersion(), req->isClose()));

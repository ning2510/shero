...
int main(int argc, char* argv[]) {
    ...
    EventLoop loop;
    HttpServer server(&loop, Address(9999), "HttpServer");
    server.setThreadNums(numThreads);

    HttpDispatch *dispatch = server.getHttpDispatch();
    ...
    dispatch->addServlet("/", 
        [](const HttpRequest& req, HttpResponse* res) {
            ...
            res->setReason(HttpStatusToString(HttpStatus::OK));
            res->setHeader("Content-Type", "text/html");
            res->setHeader("Server", "Shero");
            res->setBody("<html><head><title>This is title</title></head>"
                "<body><h1>Hello</h1>This is Shero</body></html>");
        }
    );

    dispatch->addServlet("/favicon.ico", 
        [](const HttpRequest& req, HttpResponse* res) {
            res->setStatus(HttpStatus::OK);
            res->setReason(HttpStatusToString(HttpStatus::OK));
            res->setHeader("Content-Type", "image/png");
            res->setHeader("Server", "Shero");
            res->setBody(string(favicon, sizeof(favicon)));
        }
    );
